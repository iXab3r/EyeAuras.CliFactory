...
test("YouTrack create and configure reject case collisions before authentication, then accept a distinct profile", async (t) => {
  ...
  let calls = 0;
  server.use(http.get(`${url}/api/users/me`, ({ request }) => {
    calls++;
...
test("YouTrack delete fails closed on legacy case collisions without changing files or credentials", async (t) => {

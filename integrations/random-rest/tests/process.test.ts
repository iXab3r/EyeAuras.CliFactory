...
test("packaged bin: help, configuration, HTTP output, errors and JSON-RPC", async (context) => {
  ...
  const run = (argv: string[], input = "") =>
    spawnSync(process.execPath, ["--import", preload, bin, ...argv], {
      env: { ...process.env, RANDOM_CLI_TEST_ROOT: root },
      input,
      encoding: "utf8",
      windowsHide: true,
      timeout: 30_000,
    ...
    [
      {
        ...
        id: 1,
        method: "cli.execute",
        params: { argv: ["integers"] },
      },
      {
        jsonrpc: "2.0",
        id: 2,
        method: "cli.execute",
        params: { argv: ["sequence", "--max", "3"] },
      },
    ]
  ...
  assert.deepEqual(
    frames.map((frame) => frame.result),
    [{ values: [1] }, { values: [3, 2, 1] }],
  );

...
export class RandomHttpClient implements RandomClient {
  ...
  readonly #state: RandomRequestState;

  constructor(options: RandomHttpClientOptions) {
    this.#state = options.state ?? new RandomRequestState();
  ...
  async integers(
    ...
    return this.#generate(
      "integers/",
      {
        num: String(validated.count),
        min: String(validated.min),
        max: String(validated.max),
        col: "1",
        base: "10",
        format: "plain",
        rnd: "new",
  ...
  #generate(
    ...
    return this.#state.run(async () => {
      ...
      if (quota < 0) throw this.#state.exhausted();
      const text = await this.#text(path, parameters, signal);
      const result = parseRandomValues(text, range, count, unique);
      if (!result)
        throw new Error(
  ...
  async #text(
    ...
    try {
      ...
      if (text.trimStart().startsWith("Error:")) {
        throw new Error(
          "RANDOM.ORG returned a service error. No automatic retry was made.",
        );
      }
      return text;
    } catch (error) {

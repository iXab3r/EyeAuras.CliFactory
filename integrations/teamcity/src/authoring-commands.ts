...
export function createAuthoringCommands(
  ...
  function extensions(kind: ExtensionKind): CommandDefinition {
    ...
    return command(kind, "Manage disabled-by-default extensions; property values are redacted", [
      ...
      leaf("list <job-id>", "List in server order", Permission.ReadOnly, (c, { args }) =>
        c.listExtensions(kind, args["job-id"]),
      ),
      ...
      leaf(
        ...
        Permission.Update,
        (c, { args, options }) => c.createExtension(kind, args["job-id"], input(options)),
        extensionOptions,
      ),
      leaf(
  ...
  const snapshots = command(
    ...
    [
      ...
      leaf(
        ...
        Permission.ReadOnly,
        (c, { args }) => c.listSnapshotDependencies(args["job-id"]),
      ),
      leaf(
        "show <job-id> <dependency-id>",
        "Show a dependency without property values",
        Permission.ReadOnly,
        (c, { args }) => c.getSnapshotDependency(args["job-id"], args["dependency-id"]),
      ),
      leaf(
        "create <job-id>",
        ...
        Permission.Update,
        (c, { args, options }) =>
          c.createSnapshotDependency(args["job-id"], snapshotInput(options)),
        snapshotOptions,
      ),
      leaf(
      ...
      leaf(
        ...
        Permission.Update,
        (c, { args }) =>
          c.deleteSnapshotDependency(args["job-id"], args["dependency-id"]),
      ),
    ],
  );
  const templates = command("templates", "Manage attachments to existing templates", [
    ...
    leaf(
      ...
      Permission.ReadOnly,
      (c, { args }) => c.getAttachedTemplate(args["job-id"], args["template-id"]),
    ),
    leaf(
      "list <job-id>",
      "List attached templates in priority order",
      Permission.ReadOnly,
      (c, { args }) => c.listTemplates(args["job-id"]),
    ),
    leaf(
      "attach <job-id>",
  ...
  const steps = command("steps", "Manage ordered build steps", [
    ...
    leaf("list <job-id>", "List steps in execution order", Permission.ReadOnly, (c, { args }) =>
      c.listSteps(args["job-id"]),
    ),
    leaf(
      "show <job-id> <step-id>",
      "Show a safe step summary",
      Permission.ReadOnly,
      (c, { args }) => c.getStep(args["job-id"], args["step-id"]),
    ),
    leaf(
      "create <job-id>",
      "Add a build step",
      Permission.Update,
      (c, { args, options }) => c.createStep(args["job-id"], stepInput(options)),
      stepOptions,
    ),
    leaf(
      ...
      Permission.Update,
      (c, { args, options }) =>
        c.replaceStep(args["job-id"], args["step-id"], stepInput(options)),
      stepOptions,
    ),
    leaf("delete <job-id> <step-id>", "Delete one build step", Permission.Update, (c, { args }) =>
      c.deleteStep(args["job-id"], args["step-id"]),
    ),
  ]);
  ...
  const vcs = command("vcs", "Manage existing VCS root attachments", [
    ...
    leaf(
      ...
      Permission.ReadOnly,
      (c, { args }) => c.listVcsEntries(args["job-id"]),
    ),
    leaf(
      "show <job-id> <root-id>",
      "Show one root attachment",
      Permission.ReadOnly,
      (c, { args }) => c.getVcsEntry(args["job-id"], args["root-id"]),
    ),
    leaf(
      "attach <job-id>",
    ...
    leaf(
      ...
      Permission.Update,
      (c, { args }) => c.detachVcsRoot(args["job-id"], args["root-id"]),
    ),
    command("checkout-rules", "Inspect or update checkout rules", [
      leaf("show <job-id> <root-id>", "Show checkout rules", Permission.ReadOnly, (c, { args }) =>
        c.getCheckoutRules(args["job-id"], args["root-id"]),
      ),
      leaf(
        "set <job-id> <root-id>",
        ...
        Permission.Update,
        (c, { args, options }) =>
          c.setCheckoutRules(args["job-id"], args["root-id"], text(options, "rules")),
  ...
  const projects = [
    ...
    leaf(
      ...
      Permission.Update,
      (c, { args }) =>
        c.setProjectField(args.id, args.field, args.value),
    ),
    leaf(
      "move <id>",
      "Move a project to a parent",
      Permission.Update,
      (c, { args, options }) => c.moveProject(args.id, text(options, "parent")),
      [parentOption],
    ),
    leaf(
      "delete <id>",
      ...
      Permission.Update,
      (c, { args }) => c.deleteProject(args.id),
    ),
  ];
  const jobs = [
    ...bulk.jobs,
    ...
    leaf(
      ...
      Permission.Update,
      (c, { args }) => c.setJobField(args.id, args.field, args.value),
    ),
    leaf(
      "move <id>",
      "Move a job to a project",
      Permission.Update,
      (c, { args, options }) => c.moveJob(args.id, text(options, "project")),
      [projectOption],
    ),
    leaf(
      "delete <id>",

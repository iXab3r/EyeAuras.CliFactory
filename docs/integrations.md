...
2. Define profile fields for connection identity: usually URL plus tenant, organization, or
   project when those values select a different service realm.
...
const cli = createCli({
  ...
  profile: {
    ...
    fields: [{ name: "url", flags: "--url <url>", description: "API URL" }],
  },

# Functional v1 verification — formal release pending

Final local verification accepts **118 REST operations** (98 ReadOnly, 20 Update) and the separate
issue-attachment download. The command tree has **117 service leaves**: two direct-flag pairs share
REST declarations, and download adds one ReadOnly leaf without becoming operation 119. Earlier
preparation sections are decision history; the functional checklist below now reflects this result.

- Root, source lead and independent verification passed **400 tests** (Core 33, TeamCity 39, YouTrack 328),
  with no failures or skips. Final authentication and download security corrections are covered.
- The packaged CLI, normal profile and OS keyring passed **24 fixed ReadOnly GET checks**, with no
  failures/skips. This proof performed no live binary downloads or mutations; download safety and
  failure behavior were verified offline with synthetic fixtures.
- Independent **AR118 authoring/simplicity PASS** has no remaining corrections and includes all
  derived-download costs. The retained issue/article multipart helper saves 8 net source lines with
  equivalent behavior and full setup cost counted; no overall batch reduction is implied.
- Evidence remains local and uncommitted in `EyeAuras.CliFactory-1`, branch `feature/youtrack-v1`:
  `research/review/block-118-technical-review.md`, `research/authoring-baseline/ar118-review.md`, and
  `research/authoring-baseline/snapshots/ar118-final.json` under `.workspace/workstreams/youtrack-rest/`.
  The final snapshot SHA-256 is `594bc11c7802b24f287fabac0da278e3feef6bb3c753f888836ee379e89f1e32`;
  all 99 saved/current file hashes matched. No unpushed GitHub blob link is implied.

V2 remains owned by [Issue #7](https://github.com/iXab3r/EyeAuras.CliFactory/issues/7). **This Issue stays
open and the workstream stays active pending formal release evidence**: a linked closing commit/PR,
CI and required release/privacy/close-out gates. Functional verification does not invent those results.

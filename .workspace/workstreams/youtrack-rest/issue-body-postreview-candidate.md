# Functional v1 and post-review status — formal release pending

Accepted coverage remains **118 REST operations** (98 ReadOnly, 20 Update) plus the separate
issue-attachment download. The tree has **117 service leaves**: two direct-flag pairs share REST
declarations, and download adds a ReadOnly leaf without becoming operation 119. Scope, inventory
comments and historical decisions below are unchanged.

- Local commits now contain the work: v1 `10d7fee2cbce13d90bf59a82f9946962ea69218b`, independent
  review fixes `3df5066f8d3cc1038570bf6005db32aa4ff47655`, and the accepted shared-authoring change
  `869e7aa9aa5b3c8f2c401f08b60f40c1cce87e47`. These commits remain unpushed; no remote blob or CI
  result is implied. Full-tree/staged privacy gates passed before each source commit.
- Latest post-format repository verification passed **418/418 tests** (Core 44, TeamCity 41,
  YouTrack 333), zero failures/skips. The equivalent type-only build before formatting passed all
  **24 fixed ReadOnly GET checks** through the packaged CLI, normal profile and OS keyring, with
  no failures/skips. No live binary download, mutation or raw payload capture was performed.
- Independent post-commit review closed four correctness findings; the profile isolation contract
  and its functional acceptance are in [Issue #9](https://github.com/iXab3r/EyeAuras.CliFactory/issues/9).
  The shared positional-argument inference change passed independent technical and authoring review
  under [Issue #10](https://github.com/iXab3r/EyeAuras.CliFactory/issues/10). It removed 129 redundant
  wrappers across actual TeamCity and YouTrack consumers. Current production cost is **6,955 lines**
  (+51 Core versus the corrected baseline), tests/support 9,072 and proof 513. This is simpler, safer
  typing, not a LOC reduction; production characters grew by 1,108 overall.
- Original AR118 acceptance remains historical evidence: **400 tests**, AR118 PASS, 99 matched file
  hashes and snapshot SHA `594bc11c7802b24f287fabac0da278e3feef6bb3c753f888836ee379e89f1e32`.
  Its equivalent multipart helper saved 8 net lines at that stage. The subsequent accepted PV3
  106-file manifest SHA is `99b84464feeaf8d6cbff47c9c41796464bc6968e0671bc99e39c6b1549170fb7`.
  Evidence is recorded in `.workspace/workstreams/youtrack-rest/research/` under
  `final-functional-acceptance.md`, `pv2-review-receipt.md`, `pv3-review-closeout.md` and their
  linked independent reports. The worktree remains `EyeAuras.CliFactory-1`, `feature/youtrack-v1`.

The requested commit/review/shared-authoring cycle is complete; no additional production work is
required for that accepted scope. [Issue #7](https://github.com/iXab3r/EyeAuras.CliFactory/issues/7)
owns v2. **This Issue remains OPEN and the overall workstream active for formal release evidence**:
linked closing commit/PR, CI and required close-out gates. No push, merge, CI or release is claimed.

# Outcome and baseline — historical research decision
...
# First implementation slice — historical planning decision
...
- [x] All P0/P1 mapped leaves are implemented using the shared CliFactory, without parallel legacy
  paths or service-specific logic in core. Recursive help accurately advertises their arguments.
- [x] MSW at native fetch covers each operation's method/path, encoded IDs/query/body, minimal
  response parsing, errors, and empty results; fixtures contain synthetic data only.
- [x] Every Update leaf has an explicit denial-before-fetch assertion and successful/remote-reject
  cases. ReadOnly POST exceptions work with Update disabled; profile permission isolation holds.
- [x] Auth tests prove validation before secure persistence, unavailable-keyring failure without
...
- [x] Pagination/projection tests cover documented offset and cursor modes, parameter validation,
  empty/final pages, count `-1`, and no hidden follow-up requests or mutation retry.
- [x] Real CLI/process evidence covers packaging, human output, one JSON value, stderr/exit status,
  and persistent JSON-RPC. Tests inject supported AppArguments dependencies rather than adding
  a public environment override.
- [x] A separate optional local proof invokes only a fixed bounded ReadOnly command inventory
...
- [x] Focused YouTrack tests and `npm test` pass; core changes, if any, also prove an affected TeamCity

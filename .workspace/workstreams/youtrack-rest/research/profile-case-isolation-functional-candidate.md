...
The correction is implemented and independently accepted in local commit
`3df5066f8d3cc1038570bf6005db32aa4ff47655`. Final repository tests passed **414/414**
(Core 40 / TeamCity 41 / YouTrack 333), with zero failures or skips. Four actual TeamCity/YouTrack
consumer regressions passed. Independent Windows verification exercised all eight profile-store
methods plus CLI delete/configure/auth logout, preserving configuration bytes, sentinel files and
credentials when collision handling rejected the operation. Canonical profile documentation now
states the portable uniqueness rule, exact-name behavior and explicit manual recovery requirement.

Full-tree/staged privacy review passed before the fixing commit. The commit is local and has not
been pushed; no remote commit link or CI result is claimed. **Keep this Issue open** until formal
closure evidence is linked. The workstream receipt is `research/pv2-review-receipt.md` under the
existing YouTrack workstream. No real user data or keyring was used for these regression tests.

## Original finding and contract
...
- [x] Offline profile-store create tests reject differently cased duplicate ASCII names on every
      platform and leave the original document, active profile and permissions unchanged.
- [x] Offline load tests reject pre-existing colliding documents before save/delete/secret mutation;
      a single mixed-case profile still loads and round-trips without normalization.
- [x] Actual CLI `profile create`, `profile configure` and `profile delete` paths exercise the shared
...
- [x] TeamCity and YouTrack consumer regressions prove the shared lifecycle, including a valid
...
- [x] Error output is actionable and contains no profile values, credentials or private service
...
- [x] Focused affected tests, repository `npm test`, independent review and full tracked-tree/staged
      privacy checks passed for the local fixing commit recorded above.
- [ ] Link the correcting commit and required CI evidence, then complete formal Issue/workstream
      closure. Local functional acceptance alone does not satisfy this condition.

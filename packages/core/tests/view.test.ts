...
import { command, createCli, recordView, tableView } from "../src/index.js";
...
test("tables keep identifiers whole and shrink only secondary columns to the width", () => {
  ...
  const footer = tableView<Row, { items: Row[] }>({
    rows: (value) => value.items,
    columns: [{ header: "ID", value: (row) => row.id }],
    footer: (value) => `Shown: ${value.items.length}`,
  });
  assert.equal(renderView(footer, { items: rows }, context), "ID\n101\n1234567890\n\nShown: 2");
});

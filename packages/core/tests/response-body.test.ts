...
test("bounded bytes retain split UTF-8 and BOM without selecting a decoder", async () => {
  ...
  assert.deepEqual(Buffer.from(await readResponseBody(input, { maxBytes: expected.length })), expected);
...
test("absent and empty bodies succeed, including a declared zero length", async () => {
  for (const input of [new Response(null), response([]), response([], { "content-length": "0" })]) {
    assert.equal((await readResponseBody(input, { maxBytes: 1 })).byteLength, 0);
...
test("bounds validate safe integers and count chunked actual bytes", async () => {
  for (const maxBytes of [0, -1, 1.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1]) {
    ...
    await assert.rejects(readResponseBody(input, { maxBytes }), safeFailure);
    assert.equal(input.body?.locked, false);
  }
  const input = response([Uint8Array.of(1, 2), Uint8Array.of(3, 4)]);
  await assert.rejects(readResponseBody(input, { maxBytes: 3 }), safeFailure);
  assert.equal(input.body?.locked, false);
});
...
test("declared lengths reject invalid, excessive, understated and truncated bodies", async () => {
  for (const length of ["", "-1", "+2", "1.5", "2e0", "synthetic-private", "9007199254740992", "5", "1", "3"]) {
    ...
    await assert.rejects(readResponseBody(input, { maxBytes: 4 }), safeFailure);
    assert.equal(input.body?.locked, false);
  }
  await assert.rejects(readResponseBody(new Response(null, {
    headers: { "content-length": "1" },
  }), { maxBytes: 4 }), safeFailure);
  const exact = response([Uint8Array.of(1, 2)], { "content-length": "02", "content-encoding": "IDENTITY" });
  assert.equal((await readResponseBody(exact, { maxBytes: 2 })).byteLength, 2);
});
...
test("encoded length is syntax-checked but only decoded bytes count toward the bound", async () => {
  ...
  assert.equal((await readResponseBody(response([Uint8Array.of(1, 2)], headers), { maxBytes: 2 })).byteLength, 2);
  await assert.rejects(readResponseBody(response([Uint8Array.of(1, 2, 3)], headers), { maxBytes: 2 }), safeFailure);
  assert.equal((await readResponseBody(response([], {
...
test("pre-abort and pending-read abort cancel and unlock without disclosing reasons", async () => {
  for (const before of [true, false]) {
    ...
    const pending = readResponseBody(input, { maxBytes: 4, signal: controller.signal });
    if (!before) controller.abort(new Error("synthetic-private-abort"));
...
test("abort during a chunk and stream failures release ownership with static errors", async () => {
  ...
  await assert.rejects(readResponseBody(aborted, { maxBytes: 4, signal: controller.signal }), safeFailure);
  ...
  await assert.rejects(readResponseBody(failed, { maxBytes: 4 }), safeFailure);
...
test("native fetch enforces decoded limits for actual compressed local HTTP responses", async (t) => {
  ...
  assert.deepEqual(Buffer.from(await readResponseBody(overhead, { maxBytes: tiny.length })), tiny);
  ...
  assert.deepEqual(Buffer.from(await readResponseBody(good, { maxBytes: small.length })), small);
  ...
  await assert.rejects(readResponseBody(oversized, { maxBytes: 1024 }), safeFailure);
...
test("owned accumulation handles empty chunks and a producer reusing one-byte storage", async () => {
  ...
  assert.deepEqual([...await readResponseBody(input, { maxBytes: expected.length })], expected);
  assert.equal(input.body?.locked, false);
});


test("omitting a response budget accepts a body beyond former integration ceilings", async () => {
  const bytes = new Uint8Array(9 * 1024 * 1024).fill(65);
  const actual = await readResponseBody(response([bytes]));
